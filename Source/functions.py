...
def tournaments(year):
	...
	for i in range(0, tourney_count):
		...
		# Singles winner info
		singles_winner_name_xpath = "//tr[@class = 'tourney-result'][" + str(
			i + 1) + "]/td/div[contains(., 'SGL:')]/a/text()"
		...
		if len(singles_winner_name_cleaned) > 0:
			...
			singles_winner_url_xpath = "//tr[@class = 'tourney-result'][" + str(
				i + 1) + "]/td/div[contains(., 'SGL:')]/a/@href"
		...
		doubles_winners_name_xpath = "//tr[@class = 'tourney-result'][" + str(
			i + 1) + "]/td/div[contains(., 'DBL:')]/a/text()"
		...
		doubles_winners_url_xpath = "//tr[@class = 'tourney-result'][" + str(
			i + 1) + "]/td/div[contains(., 'DBL:')]/a/@href"
...
# noinspection SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,SpellCheckingInspection,PyPep8,PyPep8,PyPep8,PyStatementEffect,PyUnusedLocal,PyUnusedLocal,PyUnusedLocal,PyUnusedLocal,PyUnusedLocal
def scrape_tourney(tourney_url_suffix):
	...
	for i in range(0, tourney_round_count):
		...
		for j in range(0, round_match_count):
			...
			# Winner
			winner_name_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-name')][1]/a/text()"
			winner_name_parsed = xpath_parse(tourney_tree, winner_name_xpath)
			
			winner_url_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-name')][1]/a/@href"
			...
			# Loser
			loser_name_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-name')][2]/a/text()"
			loser_name_parsed = xpath_parse(tourney_tree, loser_name_xpath)
			
			loser_url_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-name')][2]/a/@href"
			...
			# Seeds
			winner_seed_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-seed')][1]/span/text()"
			...
			loser_seed_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-seed')][2]/span/text()"
			...
			# Match score
			match_score_text_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-score')]/a/node()"
			match_score_text_parsed = xpath_parse(tourney_tree, match_score_text_xpath)
			
			if len(match_score_text_parsed) > 0:
				
				# Tiebreaks
				tiebreaks_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
					j + 1) + "]/td[contains(@class, 'day-table-score')]/a/sup/text()"
				...
				# Store data
				match_data.append(
					[tourney_round_name, round_order, match_order, winner_name, winner_player_id, winner_slug,
					 loser_name, loser_player_id, loser_slug, winner_seed, loser_seed, match_score_tiebreaks,
...
def scrape_match_stats(match_url_suffix):
	...
	# Store data
	output = [match_id, match_url_suffix, match_time, match_duration, winner_aces, winner_double_faults,
...
def synchronous(url_array, parsing_function, tourney_index, tourney_slug):
	...
	for match_url in url_array:
		...
		sys.stdout.write(
			'\r' + tourney_index + spacing1 + '    ' + tourney_slug + spacing2 + '    ' + current_count + "/" + str(
				total_matches) + " (" + str(percent_completed) + ")")
		sys.stdout.flush()
