	...
	for i in range(0, tourney_round_count):
		...
		for j in range(0, round_match_count):
			...
			match_score_text_xpath = "//table[contains(@class, 'day-table')]/tbody[" + str(i + 1) + "]/tr[" + str(
				j + 1) + "]/td[contains(@class, 'day-table-score')]/a/text()"
...
for h in range(int(start_year), int(end_year) + 1):
	...
	for i in range(0, len(tourney_urls_scrape)):
		if len(tourney_urls_scrape[i]) > 0:
			...
			print(year + '    ' + str(tourney_data_scrape[i][1]) + spacing1 + '    ' + tourney_data_scrape[i][
				2] + spacing2 + ' ' + str(len(match_data_scrape)))
		
		filename = "match_scores_" + start_year + "-" + end_year + str(current_time) + '.csv'
