from functions import html_parse_tree, xpath_parse, synchronous, scrape_match_stats, array2csv, format_spacing
...
for year in years:
	...
	print(
		'')
	print(
		'Collecting match stats data for ' + '\x1b[0;32;40m' + str(tourney_count) + '\x1b[0m' + ' tournaments:')
	print(
		'')
	print(
	...
	for i in range(int(start_index), tourney_count):
		...
		if len(match_stats_url_suffixes) > 0:
			...
			print(
				tourney_index + spacing1 + '    ' + tourney_slug + spacing2 + '    Match stats URL problems')
			
		# STEP 3: Output to CSV
		filename = f'match_stats_{start_year}-{end_year}_{current_time}.csv'
